...
ALTER TABLE itresume9698519.mobile_phones
ADD COLUMN manufacturer_id INT,
ADD CONSTRAINT fk_manufacturer
    FOREIGN KEY (manufacturer_id)
    REFERENCES itresume9698519.manufacturer(id)
    ON UPDATE CASCADE
    ON DELETE SET NULL;
    
UPDATE itresume9698519.mobile_phones mp
SET manufacturer_id = (
    SELECT m.id
    FROM itresume9698519.manufacturer m
    WHERE m.name = mp.manufacturer
);
